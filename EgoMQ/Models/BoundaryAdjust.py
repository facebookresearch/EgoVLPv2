...
class BoundaryAdjust(nn.Module):
    ...
    def forward(self, loc_box, feat_frmlvl):
        ...
        # Starts
        starts = torch.clamp((loc_box[:,:,0] / self.base_stride).to(dtype=torch.long), min=0, max=(self.tscale / self.base_stride)-1).long()
        ...
        # Ends
        ends = torch.clamp((loc_box[:,:,1] / self.base_stride).to(dtype=torch.long), min=0, max=(self.tscale / self.base_stride)-1).long()
        ends_left = torch.clamp(((loc_box[:,:,1] - boundary_length) / self.base_stride).to(dtype=torch.long), min=0, max=(self.tscale / self.base_stride)-1).long()
        ends_right = torch.clamp(((loc_box[:,:,1] + boundary_length) / self.base_stride).to(dtype=torch.long), min=0, max=(self.tscale / self.base_stride)-1).long()
