...
from trainer.trainer_charades import Multi_Trainer_dist_Charades
...
parser.add_argument('--task_names', default='EgoNCE_MLM_ITM', type=str, help='Task_Names')
...
def main_worker(gpu, args):
    ...
    trainer = Multi_Trainer_dist_Charades(args, model, loss, metrics, optimizer, scheduler, gpu,
                      config=config,
...
def init_dataloaders(config, module_data):
    ...
    if "type" in config["data_loader"] and "args" in config["data_loader"]:
        ...
        config['data_loader']['args'] = replace_nested_dict_item(config['data_loader']['args'], 'batch_size', 1) ## Code is wrong for batch_size != 1
